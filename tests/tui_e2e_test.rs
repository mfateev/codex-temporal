...
async fn tui_session_smoke_test() {
    match tokio::time::timeout(Duration::from_secs(300), tui_session_smoke_test_inner()).await {
        Ok(()) => {}
        Err(_) => panic!("tui_session_smoke_test timed out after 300s"),
    }
}
...
#[tokio::test]
async fn tui_tool_approval_test() {
    match tokio::time::timeout(Duration::from_secs(300), tui_tool_approval_test_inner()).await {
        Ok(()) => {}
        Err(_) => panic!("tui_tool_approval_test timed out after 300s"),
    }
}

async fn tui_tool_approval_test_inner() {
    std::env::var("OPENAI_API_KEY")
        .expect("OPENAI_API_KEY must be set to run E2E tests");
    ...
    .identity("tui-approval-test-client")
    .build();
    let telemetry_options = TelemetryOptions::builder().build();
    let runtime_options = RuntimeOptions::builder()
        .telemetry_options(telemetry_options)
    ...
    std::thread::spawn(move || {
        ...
        rt.block_on(async move {
            ...
            .identity("tui-approval-test-worker")
            .build();
    ...
    // --- Create session and submit a tool-using prompt ---
    let session = new_session(&client, "gpt-4o");

    // Use OnRequest approval policy so tool calls require explicit approval.
    session
        .submit(user_turn_op(
    ...
    assert!(
        ...
        "expected SessionConfigured before ExecApprovalRequest"
    );
    ...
    // --- Wait for TurnComplete, auto-approve any subsequent tool calls ---
    ...
    loop {
        ...
        match &event.msg {
            ...
            EventMsg::ExecApprovalRequest(req) => {
                // Auto-approve any further tool calls the model makes.
                session
                    .submit(Op::ExecApproval {
