...
#[workflow_methods]
impl AgentWorkflow {
    /// Emit an event and bump the state version atomically.
    ...
    pub fn new(_ctx: &WorkflowContextView, input: AgentWorkflowInput) -> Self {
        ...
        if let Some(ref state) = input.continued_state {
            return Self {
                ...
                approval_policy_override: state.overrides.approval_policy,
                model_override: state.overrides.model.clone(),
                effort_override: state.overrides.effort,
                summary_override: state.overrides.summary,
                personality_override: state.overrides.personality,
